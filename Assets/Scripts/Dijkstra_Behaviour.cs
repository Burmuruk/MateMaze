...
public class Dijkstra_Behaviour : MonoBehaviour
{
    public GameObject nodeSprite;
    CostGraph costGraph;
    PriorityQueue<int, Vector3> frontier;
    Dictionary<Vector3, Vector3> cameFrom;
    Dictionary<Vector3, int> costSoFar;
    ...
    void Start()
    {
        CreateGraphSprites();
        SetStartNode();
        InvokeRepeating("Dijkstra_Algorithm", 1f, 0.1f);
    }
    ...
    void Dijkstra_Algorithm()
    {
        if (frontier.Count > 0)
        {
            ...
            if (current == endNode)
            {
                CancelInvoke("Dijkstra_Algorithm");
                ReconstructPath();
            }
            ...
            {
                int newCost = costSoFar[current] + costGraph.Cost(current, next);
